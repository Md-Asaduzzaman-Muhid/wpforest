jQuery(document).ready(function($) {
    'use strict';
    var this_obj = travel_tourism_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.travel-tourism-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message travel-tourism-plugin-install');
                button.addClass('button-primary travel-tourism-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.travel-tourism-recommended-plugins .travel-tourism-activation-note').css('display','block');
    ...
    $(document).on('click', '.travel-tourism-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .travel-tourism-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .travel-tourism-recommended-plugins').css('display','none');
    });
